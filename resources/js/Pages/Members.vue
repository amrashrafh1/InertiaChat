                                ...
                                <table class="table-auto w-full text-center">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="px-4 py-2">Add Friend</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for='(member, index) in members' :key="index">
                                            ...
                                            <td class="border px-4 py-2">
                                                <a :disabled='!member.isFriend' @click='addFriend(member.email)' :class="!member.isFriend ? 'bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded cursor-pointer' : 'inline-block px-6 py-2 border-2 border-gray-200 text-gray-200 font-medium text-xs leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out'"
                                                    class=''>Add
                                                    friend</a>
                                            </td>
                                        </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            addFriend(email) {
                this.$inertia.post(route('add_friend'), {
                    email: email,
                    onSuccess: () => this.toast.success("Friend added successfully")
                })
            }
        },
    }
